...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (!this.inLink && this.options.mentions && (cap = this.rules.mention.exec(src))) {
      src = src.substring(cap[0].length);
      link = this.links[cap[2]];
      href = link && link.href || cap[2];
      out += this.renderer.mention(cap[1], cap[2], href);
      continue;
    }
...
Renderer.prototype.mention = function(preceding, id, href) {
  href = this.urltransform(href)
...
function addMentions(links, mentions) {
  if (!Array.isArray(mentions)) return;
  for (var i = 0; i < mentions.length; i++) {
    var mention = mentions[i];
    if (!mention) continue;
    var href = mention.link || '';
    links[href[0] + mention.name] = {href: href};
  }
}
...
function marked(src, opt, callback) {
  ...
  if (callback || typeof opt === 'function') {
    ...
    addMentions(tokens.links, mentions);
